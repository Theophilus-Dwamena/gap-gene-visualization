...
if show_blocks:
    ...
    # NEW
    def draw_blocks_axes(ax, ordered_genes_display, gene_x_y_dict, is_temporal=False):
        ...
        for row_idx, gene in enumerate(ordered_genes_display):
            ...
            # P(x) = normalized relative expression
            # r(x_i) = |y(x_i) - min of 20 neighbours behind (or ahead for first 20)|
            alpha = 0.3
            window = 50
            n_dense = len(y_dense)
            r = np.zeros(n_dense)
            ...
            # NEW
            # normalize r to [0, 1]:
            # r > 0.1 → P = 1 (strong local relief, full boost)
            # r <= 0.1 → P = r / 0.1 (scale the 0–0.1 range to 0–1)
            # NEW
