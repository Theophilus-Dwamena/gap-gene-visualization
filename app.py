...
st.markdown("Select genes and visualization options to explore normalized expression profiles. 'cad' and 'en' options not available for temporal data source. ", unsafe_allow_html=True)
...
if "Temporal" in selected_stage_embryos:
    ...
    for gene in selected_genes:
        if gene in temporal_scaled_genes:  # mrna genes
            ...
            if show_datapoints:
                for xi, vals in zip(temp_x_pos, ys_vals):
                    ...
                    if len(vals) > 0:
                        ax.scatter([xi] * len(vals), vals, color=temporal_utils.COLORS.get(gene, "black"), alpha=0.5,
                                   s=20)

            temporal_utils.smooth_plot(ax, temp_x_pos, ys, ses, temporal_utils.COLORS.get(gene, "black"),
                                       f"{gene} (Temporal)", show_variance)

        elif gene == "eve":  # pooled Eve
            ...
            if show_datapoints:
                for xi, vals in zip(temp_x_pos, ys_vals):
                    vals = [v for v in vals if np.isfinite(v)]
                    if len(vals) > 0:
                        ax.scatter([xi] * len(vals), vals, color=temporal_utils.COLORS.get("eve", "purple"), alpha=0.5,
                                   s=20)

            temporal_utils.smooth_plot(ax, temp_x_pos, ys, ses, temporal_utils.COLORS.get("eve", "purple"),
